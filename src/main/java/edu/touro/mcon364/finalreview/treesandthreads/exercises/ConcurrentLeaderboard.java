...
public class ConcurrentLeaderboard {
    ...
    public void runSimulation(List<String> players, int scoresEach)
        ...
        ConcurrentLeaderboard lb = this; // for lambda capture
        for (String player : players) {
            pool.submit(() -> {
                for (int i = 0; i < scoresEach; i++) {
                    int score = rand.nextInt(1000); // random score between 0-999
                    lb.submitScore(new ScoreEntry(player, score, System.currentTimeMillis()));
                }
            });
        }
        pool.shutdown();
